...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        numOfTokensReq = max_length-len(sentence)
        tokens = [zero_vector] * numOfTokensReq
        extra_labels = [zero_label] * numOfTokensReq
        sentence.extend(tokens)
        labels.extend(extra_labels)
        mask = [True for i in xrange(max_length-numOfTokensReq)]
        mask.extend([False for i in xrange(numOfTokensReq)])
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        pre_embed = tf.Variable(self.pretrained_embeddings,dtype=tf.float32)
        features = tf.nn.embedding_lookup(pre_embed,self.input_placeholder)
        embeddings = tf.reshape(features,shape=(-1,self.max_length,Config.n_features*self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        h_0 =  tf.zeros(shape=[tf.shape(x)[0],self.config.hidden_size],dtype=tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN") as scope:
            for time_step in range(self.max_length):
                ...
                if time_step == 0:
                    output,state = cell(x[:,time_step,:],h_0,scope=scope)
                else:
